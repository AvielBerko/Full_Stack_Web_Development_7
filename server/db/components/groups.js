...
async function updateGroup(updated_group, user_id) {
    return new Promise((resolve, reject) => {
        db_connection.getConnection(con => {
            ...
            WHERE user_id = ? AND ${tables.GROUPS}.id = ? AND ${tables.GROUP_MEMBERS}.admin = 1 AND ${tables.GROUP_MEMBERS}.valid = 1 AND ${tables.GROUPS}.valid = 1
            ;`,
                [updated_group.name, user_id, updated_group.id],
                (error, result) => {
                    if (error) reject(error);
                    resolve(result);
                });
