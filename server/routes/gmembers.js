...
const new_gmembers_schema = Joi.object({//2 schemas because here admin is not allowed
...
router.post("/", async (req, res) => {
  ...
  if (user.id !== req.body.user_id) {
    const result = await is_admin(group_id, user.id);
  ...
  try {
    ...
    new_gmember.groupchat_id = group_id;
