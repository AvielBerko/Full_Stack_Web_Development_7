...
export default function GroupinfoUser({ user, member, setAlert }) {
  ...
  const removeUserMutation = useMutation({
    mutationFn: () =>
      leaveGroup(member.groupchat_id, member.user_id, user.token),
    onSuccess: (results) => {
        ...
        <div className="d-flex gap-2">
          {admin && Boolean(member?.admin) && member.user_id != user.id && (
            <Button
              variant="danger"
              onClick={() => {
                updateAdminMutation.mutate({ admin: false });
              }}
            >
              DeAdmatize
            </Button>
          ...
          {admin && !Boolean(member?.admin) && member.user_id != user.id && (
            <Button
              variant="primary"
              onClick={() => {
                updateAdminMutation.mutate({ admin: true });
              }}
            >
              Admatize
            </Button>
